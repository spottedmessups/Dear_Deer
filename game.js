...
function spawnStageNotes(index) {
    ...
    setTimeout(() => { notesReady = true; isMoving = false; playNote(FREQS[melody[index]], 0.3); }, 200);
}
...
async function moveStage(p) {
    ...
    if (p === pathSeq[currentStage]) {
        ...
        notesReady = false;
        startWalkAnim();
        if (mapBottom > -5888) { // Map freeze threshold
            mapBottom -= 256;
            forest.style.bottom = `${mapBottom}px`;
        } else {
            let curB = parseInt(deer.style.bottom);
...
overlay.addEventListener('touchstart', init);
...
container.addEventListener('touchstart', (e) => {
    ...
    const x = (touchX - rect.left) / (rect.width / 544);
...
playBtn.addEventListener('touchstart', (e) => { 
    e.stopPropagation(); if (!isMoving && notesReady) playNote(FREQS[melody[currentStage]]); 
});
